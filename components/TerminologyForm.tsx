import {
  ...
  Input,
  Radio, RadioGroup, SimpleGrid, Stack, Textarea
...
const TerminologyForm: FC<TerminologyFormProps> = ({ index }) => {
  ...
  return (
    <Formik
      initialValues={{ id: generateUid(), name: '', type: '', shortName: '', description: '' }}
      validationSchema={TerminologySchema}
      onSubmit={async (values, actions) => {
        const response = await mutateAsync({ ...values, mappings: [], index });
          ...
          <SimpleGrid columns={1} gap="10px" w="60%" m="auto">
            {index === "attributes" && <>
              <Field name="identifier">
                {({ field }) => (<FormControl isInvalid={errors.name && touched.name}>
              ...
              <Field name="type">
                {({ field }) => (<FormControl isInvalid={errors.name && touched.name}>
                  <RadioGroup>
                    <Stack direction="row">
                      <Radio  {...field} value="gender">Gender</Radio>
                      <Radio  {...field} value="address">Address</Radio>
                      <Radio  {...field} value="given">Given Name</Radio>
                      <Radio  {...field} value="family">Family Name</Radio>
                      <Radio  {...field} value="birthDate">Birth Date</Radio>
                      <Radio  {...field} value="extension">Extension</Radio>
                      <Radio  {...field} value="telecom">Telecom</Radio>
                      <Radio  {...field} value="maritalStatus">Marital Status</Radio>
                    </Stack>
                  </RadioGroup>
                </FormControl>)
                }
            ...
            <Field name="description">
              {({ field }) => (
                <FormControl isInvalid={errors.description && touched.description}>
                  ...
                  <Textarea {...field} rows={2} id="description" placeholder="Description" />
