...
const TerminologyDetails: FC<TerminologyProps> = ({ id, index }) => {
  ...
  const makeIdentifier = async (data: any, value: boolean) => {
    mutateAsync({ ...data, identifier: value, index: index, idField: "id" });
  };
        ...
        <Box>
          ...
          {index === "entities" && (
            <Box mt="20px">
              <RadioGroup
                value={data.type}
                onChange={(e: any) => changeAttributeType(data, e)}
              >
                <Stack direction="row">
          ...
          {index === "attributes" && (
            <Box mt="20px">
              <Checkbox
                ...
                onChange={(e: any) => makeIdentifier(data, e.target.checked)}
              >
                Identifier
              </Checkbox>
              <RadioGroup
              ...
              {data && data.mappings ? (
                data.mappings.map((mapping: any) => (
                  <Tr key={mapping.code}>
                    ...
                    <Td>{mapping.code}</Td>
                    <Td>
                      <Button onClick={() => deleteMapping(mapping)}>
                        Delete
                      </Button>
                    </Td>
                  </Tr>
          ...
          <Modal isOpen={isOpen} onClose={onClose} isCentered>
            ...
            <Formik
              ...
              validationSchema={
                data.type === "extension" ? ExtensionSchema : MappingSchema
              }
              onSubmit={async (values, actions) => {
                let mappings = data.mappings || [];
                mappings = [...mappings, values];
                const updated = { ...data, mappings, index, idField: "id" };
                    ...
                    <ModalBody>
                      <SimpleGrid columns={1} gap="10px">
                        <Field name="system">
                          {({ field }) => (
                            <FormControl
                              isInvalid={errors.system && touched.system}
                            >
